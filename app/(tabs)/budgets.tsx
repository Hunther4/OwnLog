  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      for (const cat of categories) {
        ...
        results.push({ id: cat.id, budget: budget ?? 0, spending });
      }
    ...
    return (
      <SafeAreaView style={[styles.safeArea, { backgroundColor: palette.background }]}>
        <View style={styles.centered}>
          <Text allowFontScaling style={{ color: palette.text }}>Cargando presupuestos...</Text>
        </View>
      </SafeAreaView>
    );
  }
            ...
            return (
              <View style={[styles.budgetItem, { backgroundColor: palette.card }]}>
                <View style={styles.itemLeft}>
                  ...
                  <View>
                    <Text allowFontScaling style={[styles.catName, { color: palette.text }]}>{item.nombre}</Text>
                    <Text allowFontScaling style={[styles.spentText, { color: isOver ? palette.expense : palette.textSecondary }]}>
                ...
                {editingId === item.id ? (
                  <View style={styles.editContainer}>
                    ...
                    >
                      <Text style={[styles.saveButtonText, { color: palette.white }]}>✓</Text>
                    </TouchableOpacity>
                  </View>
                ) : (
                  <TouchableOpacity 
                    ...
                    style={[styles.editButton, { minWidth: 44, minHeight: 44 }]}
                  >
                    <Text allowFontScaling style={[styles.editButtonText, { color: palette.primary }]}>Editar</Text>
                  </TouchableOpacity>
                )}
              </View>
            );
