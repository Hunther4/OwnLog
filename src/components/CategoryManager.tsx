...
const CategoryForm = ({ category, onSave, onCancel, palette }: CategoryFormProps) => {
  ...
  const [cycle, setCycle] = useState<'daily' | 'weekly' | 'monthly' | 'yearly'>(
    (category.cycle as 'daily' | 'weekly' | 'monthly' | 'yearly') || 'monthly'
  );
  ...
  return (
    ...
    >
<Text allowFontScaling={true} style={[styles.formTitle, { color: palette.text }]}>
...
<Text allowFontScaling={true} style={[styles.budgetLabel, { color: palette.text }]}>
          ...
          <View style={styles.budgetRow}>
            ...
            >
<Text allowFontScaling={true} style={[styles.cycleButtonText, { color: palette.text }]}>
      ...
      <View style={styles.typeRow}>
        ...
        >
<Text allowFontScaling={true} style={{ color: tipo === 'ingreso' ? '#fff' : palette.text }}>
              Ingreso
            </Text>
        ...
        >
<Text allowFontScaling={true} style={{ color: tipo === 'egreso' ? '#fff' : palette.text }}>
              Egreso
            </Text>
...
<Text allowFontScaling={true} style={{ color: palette.textSecondary }}>
              Cancelar
            </Text>
        ...
        >
<Text allowFontScaling={true} style={styles.saveButtonText}>
              Guardar
            </Text>
...
export const CategoryManager: React.FC = () => {
  const { categories, addCategory, updateCategory, deleteCategory, themeMode } = useBoundStore();
  ...
  const handleSave = async (cat: Omit<Category, 'id'>) => {
    if (editingCategory?.id) {
      ...
      if (cat.budget !== undefined) {
        await useBoundStore.getState().setBudget(editingCategory.id, cat.budget || 0);
      }
    } else {
      const id = await addCategory({ ...cat, activa: true });
      if (cat.budget !== undefined) {
        await useBoundStore.getState().setBudget(id, cat.budget || 0);
      }
    }
...
<Text allowFontScaling={true} style={[styles.title, { color: palette.text }]}>
              Categorías
            </Text>
        ...
        >
<Text allowFontScaling={true} style={styles.exportButtonText}>
              Exportar CSV
            </Text>
      ...
      >
<Text allowFontScaling={true} style={[styles.addTriggerText, { color: palette.primary }]}>
...
<Text allowFontScaling={true} style={[styles.itemText, { color: palette.text }]}>
...
<Text allowFontScaling={true} style={styles.deleteButtonText}>
                      🗑️
                    </Text>
