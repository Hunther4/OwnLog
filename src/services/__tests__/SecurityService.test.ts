...
describe('SecurityService', () => {
  ...
  describe('setupPin (Task 2.2)', () => {
    it('should generate salt, hash PIN, store them, and return a recovery key', async () => {
      ...
      (Crypto.digestStringAsync as jest.Mock).mockImplementation(() => {
        digestCalls++;
        return Promise.resolve(digestCalls <= 10000 ? mockHash : mockRecoveryHash);
      });
  ...
  describe('resetPinWithRecoveryKey (Task 2.4)', () => {
    it('should reset PIN if recovery key is valid', async () => {
      ...
      (Crypto.digestStringAsync as jest.Mock).mockImplementation(() => {
        digestCalls++;
        // First call: recovery key hash check, then 10,000 calls for hashPin
        if (digestCalls === 1) return Promise.resolve(storedRecoveryHash);
        // Next 10,000 calls: hashPin(newPin)
        if (digestCalls <= 10001) return Promise.resolve(mockHash);
